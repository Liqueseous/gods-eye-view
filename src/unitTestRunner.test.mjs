...
test('unit runner discovers source and server tests', () => {
  const root = fileURLToPath(new URL('..', import.meta.url));
  const files = discoverUnitTestFiles(root);
  assert.ok(files.includes('src/unitTestRunner.test.mjs'));
  assert.ok(files.includes('server/providers/transitHistory.test.mjs'));
  assert.ok(files.includes('server/providers/wikipedia/wikipedia.test.mjs'));
});
